...
app.post('/registerUser', async (req, res) => {
  const { name, lastName, numberPhone, nit, email, role } = req.body;

  // Validar los datos recibidos
  if (!name || !lastName || !numberPhone || !nit || !email || !role) {
      return res.status(400).json({ message: 'Todos los campos son requeridos.' });
  }

  try {
      // // Datos de la persona
       const personData = { name, lastName, numberPhone, nit, email, role };
...
const generatePassword = (password) => {
  ...
  return hashedPassword; // Devuelve ambas versiones
};

app.post("/CredentialsChang", async (req, res) => {
  try {
    const { id, userName, currentPassword, newPassword, confirmPassword, passwordActual } = req.body;

    console.log(req.body)
    // // Validación de los campos requeridos
     if (!id || !userName || !currentPassword || !newPassword || !confirmPassword) {
       return res.status(400).send({ error: "Todos los campos son obligatorios" });
     }

    // // Validación de las contraseñas: la nueva contraseña debe coincidir con la confirmación
     if (newPassword !== confirmPassword) {
       return res.status(400).send({ error: "La nueva contraseña y la confirmación no coinciden" });
     }

     const encry = generatePassword(currentPassword)
      // Validación de las contraseñas: la nueva contraseña debe coincidir con la confirmación
      if (passwordActual !== encry) {
       return res.status(400).send({ error: "La contraseña actual es incorrecta" });
      }

      
      const result = await changeUsernameAndPassword(id, userName, newPassword);

      if (result.success) {
       res.status(200).send({ message: "Nombre de usuario y contraseña actualizados correctamente" });
      } else {
       res.status(500).send({ error: "Error al actualizar las credenciales" });
      }
