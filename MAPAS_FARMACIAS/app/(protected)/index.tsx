...
function HomeScreen() {
  ...
  const getUserFromStorage = async () => {
    try {
      const user = await AsyncStorage.getItem('user');  // Aquí se asume que guardaste el objeto usuario bajo la clave 'user'
      if (user) {
        const userData = JSON.parse(user);  // Si lo guardaste como un objeto JSON
        console.log('Usuario recuperado:', userData);
        setUserName(userData.userName)
        setRole(userData.role)
        return userData;
      } else {
        console.log("No se encontró información del usuario en el almacenamiento.");
        return null;
      }
    } catch (error) {
      console.error("Error al recuperar el usuario del almacenamiento:", error);
      return null;
    }
  };
  ...
  const getTown = async () => {
    try {
      ...
      console.error("Error al recuperar los datos de la ciudad:", error);
    }
  };
  ...
  const getMyFarmacies = () => {
    const filteredPharmacies = usepharmacy.filter((pharmacyl: Pharmacy) => pharmacyl.userName === userName );
    setPharmacyFilter(filteredPharmacies);
    setFilterBack(true)
  };
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      ...
      <View style={styles.searchContainer}>
        ...
        />
              ...
              {/* Botón "Mis Sucursales" debajo del Picker */}
              { role === "Propietario" ? (
                <>
